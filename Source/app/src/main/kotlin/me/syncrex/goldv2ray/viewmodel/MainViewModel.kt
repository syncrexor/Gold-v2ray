...
import me.syncrex.goldv2ray.databinding.DialogConfigFilterBinding
import me.syncrex.goldv2ray.dto.*
import me.syncrex.goldv2ray.extension.toast
import me.syncrex.goldv2ray.util.*
import me.syncrex.goldv2ray.util.MmkvManager.KEY_ANG_CONFIGS
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    /*fun filterConfig(context :Context) {
        ...
        val builder = AlertDialog.Builder(context, R.style.DialogStyle).setView(ivBinding.root)
    ...
    fun filterConfig(context: Context) {
        ...
        ivBinding.spSubscriptionId.setSelection(checkedItem)
        ivBinding.etKeyword.text = Utils.getEditable(keywordFilter)
        ivBinding.root.setBackgroundColor(ContextCompat.getColor(context, R.color.customColorGoldDeepDark))
        ivBinding.etKeyword.setTextColor(ContextCompat.getColor(context, R.color.customColorGold))
        ivBinding.spSubscriptionId.setPopupBackgroundResource(R.color.customColorGoldDark)
        val builder = AlertDialog.Builder(context, R.style.DialogStyle)
        builder.setView(ivBinding.root)
            .setPositiveButton(R.string.tasker_setting_confirm) { dialogInterface: DialogInterface?, _: Int ->
                try {
                    val position = ivBinding.spSubscriptionId.selectedItemPosition
                    subscriptionId = if (listRemarks.count() - 1 == position) {
                        ""

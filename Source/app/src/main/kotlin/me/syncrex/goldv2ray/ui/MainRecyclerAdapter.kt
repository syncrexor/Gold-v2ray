...
//            }
            ...
            if(!AppConfig.LOCK_PROFILES) {
                ...
                /*holder.itemMainBinding.layoutShare.setOnClickListener {
                    AlertDialog.Builder(mActivity, R.style.DialogStyle).setItems(shareOptions.toTypedArray()) { _, i ->
                        try {
                            when (i) {
                                0 -> {
                                    if (config.configType == EConfigType.CUSTOM) {
                                        ...
                                        AlertDialog.Builder(mActivity, R.style.DialogStyle).setView(ivBinding.root)
                                            .show()
                                    }
                                }
                ...
                holder.itemMainBinding.layoutShare.setOnClickListener {
                    MaterialAlertDialogBuilder(mActivity, R.style.DialogStyle)
                ...
                holder.itemMainBinding.layoutRemove.setOnClickListener {
                    if (guid != mainStorage?.decodeString(MmkvManager.KEY_SELECTED_SERVER)) {
                        if (settingsStorage?.decodeBool(AppConfig.PREF_CONFIRM_REMOVE) == true) {
                            AlertDialog.Builder(mActivity, R.style.DialogStyle).setMessage(R.string.del_config_comfirm)

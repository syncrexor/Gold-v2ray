...
import me.syncrex.goldv2ray.dto.V2rayConfig.RoutingBean.RulesBean
import me.syncrex.goldv2ray.extension.isNotNullEmpty
import me.syncrex.goldv2ray.dto.ProfileItem
import me.syncrex.goldv2ray.fmt.HttpFmt
...
import me.syncrex.goldv2ray.dto.ConfigResult
...
import me.syncrex.goldv2ray.dto.V2rayConfig.OutboundBean.OutSettingsBean
import me.syncrex.goldv2ray.dto.V2rayConfig.OutboundBean.StreamSettingsBean
...
object V2rayConfigManager {
    ...
    private fun genV2rayMultipleConfig(context: Context, configList: List<ProfileItem>): V2rayConfig? {
        ...
        val baseTitle = context.getString(R.string.intelligent_selection)
        val remarksText = when {
            subIds.size == 1 && subIds.first().isNotEmpty() -> {
                val sub = MmkvManager.decodeSubscription(subIds.first())
                val prefix = sub?.remarks?.trim().orEmpty()
                if (prefix.isNotEmpty()) "$baseTitle: $prefix" else baseTitle
            }
            subIds.size == 1 && subIds.first().isEmpty() -> {
                val userLabel = context.getString(R.string.user)
                "$baseTitle: $userLabel"
            }
            else -> {
                baseTitle
        ...
        v2rayConfig.remarks = AppConfig.SMART_MARKER + remarksText
        //GOLDV2RAY END

...
import me.syncrex.goldv2ray.AppConfig.PREF_ALLOW_INSECURE
import me.syncrex.goldv2ray.AppConfig.PREF_CONFIRM_REMOVE
import me.syncrex.goldv2ray.dto.AssetUrlItem
import me.syncrex.goldv2ray.dto.ServerAffiliationInfo
import me.syncrex.goldv2ray.dto.SubscriptionItem
import me.syncrex.goldv2ray.AppConfig.PREF_ROUTING_RULESET
import me.syncrex.goldv2ray.dto.RulesetItem
import me.syncrex.goldv2ray.AppConfig.PREF_IS_BOOTED
import me.syncrex.goldv2ray.AppConfig.PREF_PREFER_IPV6
import me.syncrex.goldv2ray.AppConfig.PREF_SPEED_ENABLED
import me.syncrex.goldv2ray.AppConfig.PREF_START_SCAN_IMMEDIATE
...
object MmkvManager {
    ...
    fun removeServer(guid: String) {
        ...
        serverRawStorage.remove(guid) //GOLDV2RAY
    }
    ...
    //GOLDV2RAY
    fun decodeSubscriptionsNoFree(): List<Pair<String, SubscriptionItem>> {
        initSubsList()
        val subscriptions = mutableListOf<Pair<String, SubscriptionItem>>()
    ...
    //GOLDV2RAY
    fun setScanImmediate(guid: Boolean) {
        settingsStorage.encode(PREF_START_SCAN_IMMEDIATE, guid)
    }

    //GOLDV2RAY
    fun setAllowInsecure(guid: Boolean) {
        settingsStorage.encode(PREF_ALLOW_INSECURE, guid)
    }

    //GOLDV2RAY
    fun setPreferIpv6(guid: Boolean) {
        settingsStorage.encode(PREF_PREFER_IPV6, guid)
    }

    //GOLDV2RAY
    fun setSpeedEnable(guid: Boolean) {
        settingsStorage.encode(PREF_SPEED_ENABLED, guid)
    }

    //GOLDV2RAY
    fun setConfirmRemove(guid: Boolean) {
        settingsStorage.encode(PREF_CONFIRM_REMOVE, guid)
    }
}
